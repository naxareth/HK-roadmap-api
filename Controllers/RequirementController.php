...
```php
// Add Logs
private $logFile = '../logs/requirement_controller.log';
...
// Modify the add method to include logging
public function add() {
    // ...
    $this->log('Adding requirement for student ID ' . $student_id);
    // ...
    if ($this->requirementModel->add($student_id, $event_name, $event_date, $due_date, $shared, $imagePath)) {
        $this->log('Requirement added successfully for student ID ' . $student_id);
        echo json_encode(["message" => "Requirement added successfully."]);
    } else {
        $this->log('Failed to add requirement for student ID ' . $student_id);
        echo json_encode(["message" => "Failed to add requirement."]);
    }
}

// Modify the getRequirements method to include logging
public function getRequirements() {
    // ...
    $this->log('Getting requirements for token ' . $token);
    // ...
    if ($admin) {
        // ...
        $this->log('Returning all requirements for admin');
        echo json_encode($requirements);
    } else {
        // ...
        $this->log('Returning requirements for student ID ' . $student_id);
        echo json_encode($requirements);
    }
}
```